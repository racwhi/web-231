...
"use strict";
// Global variables
let cashBox = document.getElementById("cash"); //wrap string element id's with quotes
let billBox = document.getElementById("bill"); //wrap string element id's with quotes
let changeBox = document.getElementById("change"); //wrap string element id's with quotes

// Event handlers to be run when the cash or bill value changes
cashBox.addEventListener("change", runTheRegister); //function is runTheRegister and not runRegister
billBox.addEventListener("change", runTheRegister); //function is runTheRegister and not runRegister
...
function runTheRegister() {
  ...
  let changeValue = +cashBox.value - +billBox.value; // calculate the change, update

  try {
    if (changeValue < 0) {
      // alert("Cash amount doesn't cover the bill.");
...
function calcChange(changeValue) {
  ...
  changeValue -= bill5Amt * 5; //logic error
