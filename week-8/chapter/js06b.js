...
//Check if the owner name is entered on the cardfunction
...
//Check if the credit card has been selected
function validateCard() {
  let card = document.forms.payment.elements.credit[0];
  if (card.validity.valueMissing) {
    cardName.setCustomValidity("Select your credit card");
  } else {
...
function validateCVC() {
  ...
  } else if (card && card.value === "amex" && !/^\d{4}$/.test(cvc.value)) {
    cvc.setCustomValidity("Enter a 4-digit number");
  } else if (card && card.value !== "amex" && !/^\d{3}$/.test(cvc.value)) {
